...
export function ProcessingScreen({ analysisId, onComplete }: { analysisId: string, onComplete: (data: any) => void }) {
  ...
  // Determine active step based on mode and status
  ...
  const StepItem = ({ index, title, activeDesc }: { index: number, title: string, activeDesc?: string }) => {
  ...
  return (
    <div className="max-w-[600px] w-full mx-auto my-12 text-center animate-fade-in px-4">
      ...
      <Card className="bg-bg-darker border-glass-border p-6 text-left rounded-xl">
        <StepItem index={0} title="Upload original video to storage" />
        <StepItem index={1} title="Extract audio cues and chimes (Gemini)" activeDesc={mode === "gemini_only" ? "Querying gemini-2.5-flash for complete review..." : "Extracting audio cues and chimes (Gemini)"} />
        {mode !== "gemini_only" && (
          <StepItem index={2} title="Generate silent video track (FFmpeg)" />
        )}
        {mode !== "gemini_only" && (
          <StepItem index={3} title="Evaluate visuals & overlays (Gemma 31B)" />
        )}
        <StepItem index={mode === "gemini_only" ? 2 : 4} title="Compile structured score card" />
      </Card>
    </div>
  );
}
