...
export function AnalysisDashboard({ data, onReanalyze }: { data: any, onReanalyze: (id: string) => void }) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-[45%_55%] gap-8 animate-fade-in h-full px-4 mb-12">
      {/* Left Column: Player & Timeline */}
      <div className="flex flex-col gap-6">
        <div className="bg-glass-bg border border-glass-border rounded-2xl p-4 shadow-[0_8px_32px_0_rgba(0,0,0,0.4)] relative">
          <video 
            key={data.video_url}
            ref={videoRef}
            src={data.video_url}
            controls 
            preload="auto"
            className="w-full rounded-xl bg-black block aspect-[9/16] max-h-[500px] object-contain mx-auto"
          />
        ...
        <div className="bg-glass-bg border border-glass-border rounded-2xl p-6 flex-1 overflow-y-auto max-h-[600px] custom-scrollbar">
      ...
      <div className="flex flex-col gap-8 overflow-y-auto pr-2 custom-scrollbar pb-12">
        {/* Scores */}
